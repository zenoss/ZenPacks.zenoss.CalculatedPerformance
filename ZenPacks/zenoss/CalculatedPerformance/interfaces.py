...
class IAggregatingDataSourceInfo(IRRDDataSourceInfo):
    ...
    minimumInterval = schema.Int(title=_t(u'Minimum Interval (seconds)'))
    useBasisInterval = schema.Bool(title=_t(u'Use Basis Interval'))
    maximumInterval = schema.Int(title=_t(u'Maximum Interval (seconds)'))
...
class IElementPoolInfo(IComponentInfo):
    ...
    # NOTE: this field must be on the on the "fields" of the component grid for the members grid to be displayed
    numberOfMembers = Attribute("The count of the number of members in this pool")
